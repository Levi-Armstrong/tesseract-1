                             ...
                             const tesseract_scene_graph::SRDFModel::ConstPtr& srdf_model)
{
  ...
  ++revision_;
  commands_.push_back(std::make_shared<AddSceneGraphCommand>(scene_graph, ""));
...
{
  return applyCommand(std::make_shared<AddCommand>(std::make_shared<tesseract_scene_graph::Link>(std::move(link))));
}

bool Environment::addLink(tesseract_scene_graph::Link link, tesseract_scene_graph::Joint joint)
{
  return applyCommand(std::make_shared<AddCommand>(std::make_shared<tesseract_scene_graph::Link>(std::move(link)),
                                                   std::make_shared<tesseract_scene_graph::Joint>(std::move(joint))));
}
...
{
  return applyCommand(
      std::make_shared<MoveLinkCommand>(std::make_shared<tesseract_scene_graph::Joint>(std::move(joint))));
}
...
bool Environment::addSceneGraph(const tesseract_scene_graph::SceneGraph& scene_graph,
                                ...
                                const std::string& prefix)
{
  return applyCommand(std::make_shared<AddSceneGraphCommand>(
      scene_graph, std::make_shared<tesseract_scene_graph::Joint>(std::move(joint)), prefix));
}
...
bool Environment::applyAddCommand(AddCommand::ConstPtr cmd)
{
  ...
  if (!cmd->getJoint())
  {
    ...
    auto joint = std::make_shared<tesseract_scene_graph::Joint>(joint_name);
