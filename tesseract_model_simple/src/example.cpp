...
{
  using namespace tesseract_model_simple;
  using namespace tesseract_scene_graph;
  ...
  auto world_model = std::make_unique<SceneModel>(std::move(world_sg));
  auto iiwa_model = std::make_unique<SceneModel>(std::move(iiwa_sg));
  auto ur_model = std::make_unique<SceneModel>(std::move(ur_sg));
  ...
  CompositeModel composite("environment", std::move(world_model));
  composite.addModel(world_model_name, "iiwa_attach_link", std::move(iiwa_model));
  composite.addModel(world_model_name, "ur_attach_link", std::move(ur_model));

  ModelState::UPtr state = composite.getState();
  VariableSets variable_sets = state->getVariableSets();
  VariableSet iiwa_variables = variable_sets[iiwa_model_name];

  std::cout << "Model Name: " << composite.getName() << std::endl;

  std::cout << "Link Names: " << std::endl;
  for (const auto& link_name : composite.getLinkNames())
    std::cout << "    " << link_name << std::endl;
  std::cout << std::endl;

  std::cout << "Active Link Names: " << std::endl;
  for (const auto& link_name : composite.getActiveLinkNames())
    std::cout << "    " << link_name << std::endl;
  std::cout << std::endl;

  std::cout << "Joint Names: " << std::endl;
  for (const auto& joint_name : composite.getJointNames())
    std::cout << "    " << joint_name << std::endl;
  std::cout << std::endl;

  std::cout << "Active Joint Names: " << std::endl;
  for (const auto& joint_name : composite.getActiveJointNames())
    std::cout << "    " << joint_name << std::endl;
  std::cout << std::endl;
}
