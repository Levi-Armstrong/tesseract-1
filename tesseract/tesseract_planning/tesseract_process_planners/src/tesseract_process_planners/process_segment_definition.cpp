...
std::size_t ProcessSegmentDefinition::size() const
{
  return approach.size() + process.size() + departure.size();
}

bool ProcessSegmentDefinition::isApproach(const std::size_t index) const
{
  return (index < approach.size());
}

bool ProcessSegmentDefinition::isProcess(const std::size_t index) const
{
  return ((index >= approach.size()) && (index < (approach.size() + process.size())));
}

bool ProcessSegmentDefinition::isDeparture(const std::size_t index) const
{
  return ((index >= (approach.size() + process.size())) && (index < (approach.size() + process.size() + departure.size())));
}

ProcessSegmentDefinition::Iterator ProcessSegmentDefinition::begin()
{
  return Iterator(*this, 0);
}

ProcessSegmentDefinition::Iterator ProcessSegmentDefinition::end()
{
  return Iterator(*this, size());
}

tesseract_motion_planners::Waypoint::Ptr& ProcessSegmentDefinition::operator[](std::size_t index)
{
    assert(index < size());
...
const tesseract_motion_planners::Waypoint::Ptr& ProcessSegmentDefinition::operator[](std::size_t index) const
{
    assert(index < size());
    if (index < approach.size())
      return approach[index];
...
ProcessSegmentDefinition::Iterator::Iterator(ProcessSegmentDefinition& container, std::size_t pos) : pos_(pos), container_(&container)
{
...
ProcessSegmentDefinition::Iterator::difference_type ProcessSegmentDefinition::Iterator::operator-(const Iterator& rhs) const
{
